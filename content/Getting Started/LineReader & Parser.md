...
## Configuring Parser
`Parser` is an AutoLoad can be configured by you. It is located at `res://addons/diisis/parser/autoload/parser.tscn`. Implicitly, when you save a file, the parser will automatically read that file (bound by Godot project). You can override this with any DIISIS-Generated file however if you set `source_file_override`.

Here you can also adapt the look of the history.

## Configuring LineReader
`LineReader` is the other part of this equation. It is a node you can add with `CTRL + A` like any other node. Add it to the scene where you want the text from your file to be read in, or make a part of your game inherit from ``LineReader``. (The VN template does this.)

### Name Setup
When a new character starts speaking, the `name_label` will need to be updated accordingly. To give your characters proper names, you can use the exported ``LineReader.actor_config`` property. Keys should be the raw internal strings used in the [[Text#Actor Names|Speaker Stringkit]]. You can use the ``name_display`` property to give a name.

![[Pasted image 20260316205138.png]]

If no name is set, the raw string will be displayed.
...
![[Pasted image 20260316205320.png]]
...
`LineReader` is user-input driven. Within your input system, you need to call ``request_advance()`` to make the LineReader do the next thing, such as:
